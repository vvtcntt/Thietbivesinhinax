...
namespace INAX.Controllers.Admin.Product
{
    public class ProductadController : Controller
    {
        ...
        public ActionResult Create(string id)
        {
            ...
            if (ClsCheckRole.CheckQuyen(4, 1, int.Parse(Request.Cookies["Username"].Values["UserID"])) == true)
            {
                ...
                string chuoigroup = "";
                var listgroup = db.tblGroups.Where(p => p.Active == true).OrderBy(p => p.Ord).ToList();
        ...
        public ActionResult Create(tblProduct tblproduct, FormCollection Collection, string id, List<HttpPostedFileBase> uploadFile, List<HttpPostedFileBase> uploadFiles)
        {
            ...
            if (nidCate != "")
            {
                ...
                clsSitemap.CreateSitemap(tblproduct.Tag+"-dt", listprro[0].id.ToString(), "Product");
        ...
        public ActionResult Edit(int? id)
        {
            ...
            if (ClsCheckRole.CheckQuyen(4, 2, int.Parse(Request.Cookies["Username"].Values["UserID"])) == true)
            {
                ...
                var listgroup=db.tblGroups.Where(p=>p.Active==true).OrderBy(p=>p.Ord).ToList();
                      int idgroup=0;
                if(tblproduct.Group.ToString()!=null && tblproduct.Group.ToString()!="")
                {
                    idgroup   = int.Parse(tblproduct.Group.ToString());
                }
               
        
                for (int i = 0; i < listgroup.Count;i++ )
                {
                    if(listgroup[i].id==idgroup)
                    {
                        chuoigroup += "<label style=\"font-size:13px; line-height:28px; margin:0px 5px\"><input   checked=\"checked\" style=\"float:left\" type=\"radio\" name=\"rdGroups\" value=\"" + listgroup[i].id + "\" />" + listgroup[i].Name + "</label> ";
        ...
        public ActionResult Edit(tblProduct tblproduct, FormCollection collection, int? id, List<HttpPostedFileBase> uploadFile, List<HttpPostedFileBase> uploadFiles)
        {
            ...
            if (ModelState.IsValid)
            {
                if (collection["drMenu"] != "" || collection["drMenu"] != null)
                {
                    ...
                    if (collection["PriceSaleActive"] != null)
                    {
                        float PriceSaleActive = float.Parse(collection["PriceSaleActive"]);
                        tblproduct.PriceSaleActive = PriceSaleActive;
                    }
                    if (collection["rdGroups"] != null)
                    {
                        int group = int.Parse(collection["rdGroups"]);
                    ...
                    if (URL == true)
                    {
                        ...
                        clsSitemap.UpdateSitemap(StringClass.NameToTag(tblproduct.Name)+"-dt", id.ToString(), "Product");
