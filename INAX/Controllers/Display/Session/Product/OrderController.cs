...
namespace CMSCODE.Controllers.Display.Product
{
    public class OrderController : Controller
    {
            ...
            if (collection["btnOrder"] != null)
            {
                try
                {
                    ...
                    for (int i = 0; i < Sopping.CartItem.Count; i++)
                    {
                        ...
                        string gias =Convert.ToInt32( Sopping.CartItem[i].Price.ToString()).ToString();
                    ...
                    chuoihtml += "<p style=\"font-size:12px; text-align:center; margin:5px 5px\">Cơ sở 1 : " + config.Address1 + "</p>";
                    chuoihtml += "<p style=\"font-size:12px; text-align:center; margin:5px 5px\">Điện thoại : " + config.Mobile1 + " - " + config.Hotline1 + "</p>";
                    chuoihtml += "<p style=\"font-size:12px; text-align:center; margin:5px 5px\">Cơ sở 2 : " + config.Address1 + "</p>";
                    chuoihtml += "<p style=\"font-size:12px; text-align:center; margin:5px 5px\">Điện thoại : " + config.Mobile1 + " - " + config.Hotline1 + "</p>";
        ...
        //    if (Session["idProduct"] != null && Session["idProduct"] != "")
        //    {
        ...
        //    ViewBag.Description = "<meta name=\"description\" content=\" Giỏ hàng đặt hàng Thiết bị vệ sinh Inax dành cho khách hàng mua sản phẩm\"/>";
        //    ViewBag.Keyword = "<meta name=\"keywords\" content=\"Giỏ hàng của bạn\" /> ";
        //    return View(giohang);
        //}
        public ActionResult OrderIndex(string idp)
        {
            int id; int Ord; int idMenu;
            if (idp != null)
            {
                id = int.Parse(idp);
                Ord = 1;
                idMenu = int.Parse(db.tblProducts.First(p => p.id == id).idCate.ToString());
                int sl = 0;
                var Sopping = (clsGiohang)Session["giohang"];
                if (Sopping == null)
                {
